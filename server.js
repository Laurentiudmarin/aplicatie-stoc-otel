...
async function generateExcelReport(rezultateStoc) {
    ...
    // Pas 1: Pregătirea datelor o singură dată
    ...
    const redBoldFont = { name: 'Calibri', size: 11, bold: true, color: { argb: 'FFFF0000' } };
    ...
    worksheet1.eachRow((row, rowNumber) => {
        if (rowNumber > 1) {
            ...
            row.eachCell(cell => {
                cell.font = statusVal === 'Stoc Redus' ? redBoldFont : defaultFont;
    ...
    dateTabel.forEach(row => {
        let originalTip = row.tip.trim();
        let group = groupMappings[originalTip] || dataToHeaderMap[originalTip] || originalTip;
    ...
    for (let i = 0; i < maxRows; i++) {
        ...
        addedRow.eachCell({ includeEmpty: true }, (cell, colNumber) => {
            ...
            if (item) {
                ...
                cell.font = { name: 'Calibri', size: 18, bold: true }; // Aplicăm stilul și pe celulele goale
            }
        });
    }
