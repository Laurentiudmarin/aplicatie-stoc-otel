// --- server.js - COD COMPLET FINAL (cu corecție pentru legendValue) ---
...
pool.query(`CREATE TABLE IF NOT EXISTS reguli (id SERIAL PRIMARY KEY, furnizor TEXT NOT NULL, criterii TEXT NOT NULL, tip_material TEXT NOT NULL, descriere_raport TEXT NOT NULL)`).then(() => console.log('Baza de date PostgreSQL și tabelul "reguli" sunt pregătite.')).catch(err => console.error('Eroare la crearea tabelului:', err));
const app = express(); app.use(express.json()); const PORT = process.env.PORT || 3000; const upload = multer({ dest: 'uploads/' }); app.use(express.static('public'));
...
// --- FUNCȚIILE DE GENERARE A RAPOARTELOR (CU CORECȚIE) ---

async function generateExcelReport(rezultateStoc) {
    const workbook = new ExcelJS.Workbook();
    const dateTabel = Object.keys(rezultateStoc).map(cheie => {
        const [tipMaterial, codCuloare] = cheie.split('|');
        const cantitate = rezultateStoc[cheie];
        return { tip: tipMaterial, cod: codCuloare, cantitate: parseFloat(cantitate.toFixed(3)), status: cantitate >= 10 ? 'Stoc Suficient' : 'Stoc Redus' };
    }).filter(row => row.cantitate >= 1).sort((a, b) => a.tip.localeCompare(b.tip));

    // CORECȚIE: Definirea legendei la începutul funcției
    const legendValue = {
        richText: [
            { font: { bold: true, color: { argb: 'FF000000' } }, text: '* ≥10 tone: Stoc Suficient ⚫\n' },
    ...
    headerRow1.eachCell((cell) => { cell.fill = greenFill; cell.alignment = { vertical: 'middle', horizontal: 'center' }; });
    worksheet1.eachRow((row, rowNumber) => { if (rowNumber > 1 && row.getCell('status').value === 'Stoc Redus') { row.getCell('cod').font = redBoldFont; row.getCell('cantitate').font = redBoldFont; row.getCell('status').font = redBoldFont; } });
    ...
    legendCell1.value = legendValue; // Folosim variabila definită
    ...
    headerRow2.eachCell((cell) => { cell.fill = greenFill; cell.alignment = { vertical: 'middle', horizontal: 'center' }; });
    worksheet2.eachRow((row, rowNumber) => { if (rowNumber > 1 && row.getCell('status').value === 'Stoc Redus') { row.getCell('cod').font = redBoldFont; row.getCell('status').font = redBoldFont; } });
    ...
    legendCell2.value = legendValue; // Folosim aceeași variabilă
    legendCell2.alignment = { wrapText: true, vertical: 'top' };
    worksheet2.getRow(legendRowIndex2).height = 55;

    // --- Sheet 3: Stoc - UZ Extern- simplificat ---
    const worksheet3 = workbook.addWorksheet('Stoc - UZ Extern- simplificat');
    const headerSimplificat = ['SUPREM', 'NEOMAT', 'MAT 0.5', 'MAT 0.45', 'MAT 0.4', 'LUCIOS 0.5', 'LUCIOS 0.45', 'LUCIOS 0.4', 'LUCIOS 0.35', 'LUCIOS 0.3', 'ZN', '> 0.5', 'IMITATIE LEMN'];
    worksheet3.columns = headerSimplificat.map(h => ({ header: h, key: h, width: 22 }));
    worksheet3.autoFilter = { from: { row: 1, column: 1 }, to: { row: 1, column: headerSimplificat.length } };
    const headerRow3 = worksheet3.getRow(1);
    headerRow3.font = { name: 'Calibri', size: 14, bold: true };
    headerRow3.eachCell(cell => { cell.fill = greenFill; cell.alignment = { vertical: 'middle', horizontal: 'center', wrapText: true }; });
    headerRow3.height = 30;
    const groupMappings = { 'LUCIOS 0.5': 'LUCIOS', 'LUCIOS 0.45': 'LUCIOS', 'LUCIOS 0.4': 'LUCIOS', 'LUCIOS 0.35': 'LUCIOS', 'LUCIOS 0.3': 'LUCIOS', 'MAT 0.5': 'MAT', 'MAT 0.45': 'MAT', 'MAT 0.4': 'MAT', 'Imitatie Lemn SP': 'IMITATIE LEMN', 'Imitatie Lemn DP': 'IMITATIE LEMN', 'SUPREM': 'SUPREM', 'NEOMAT': 'NEOMAT', 'ZN': 'ZN', '> 0.5': 'GROSIMI MARI' };
    const pivotData = {};
    let uniqueCoduri = new Set();
    dateTabel.forEach(row => { const materialType = row.tip.trim(); let group = groupMappings[materialType] || (headerSimplificat.includes(materialType) ? materialType : null); if (group) { if (!pivotData[row.cod]) { pivotData[row.cod] = {}; } const existingStatus = pivotData[row.cod][group]; if (!existingStatus || (existingStatus === 'Stoc Redus' && row.status === 'Stoc Suficient')) { pivotData[row.cod][group] = row.status; } uniqueCoduri.add(row.cod); } });
    const sortedCoduri = Array.from(uniqueCoduri).sort();
    const blackFont = { name: 'Calibri', bold: true, color: { argb: 'FF000000' } };
    sortedCoduri.forEach(cod => { const rowData = {}; headerSimplificat.forEach(groupName => { rowData[groupName] = pivotData[cod]?.[groupName] || ''; }); const row = worksheet3.addRow(rowData); row.getCell(1).value = cod; row.getCell(1).font = { bold: true }; row.eachCell((cell, colNumber) => { if (colNumber > 1) { if (cell.value === 'Stoc Redus') { cell.font = redBoldFont; } else if (cell.value === 'Stoc Suficient') { cell.font = blackFont; } } }); });
    const legendRowIndex3 = worksheet3.lastRow.number + 2;
    ...
    legendCell3.value = legendValue; // Folosim aceeași variabilă
