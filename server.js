// --- server.js - COD COMPLET FINAL (cu toate funcționalitățile) ---
...
async function generateExcelReport(rezultateStoc) {
    ...
    worksheet3.columns = headerSimplificat.map(h => ({ header: h, key: h.replace(/\s|\./g, ''), width: 22 }));
    ...
    const blackBoldFont = { name: 'Calibri', bold: true, color: { argb: 'FF000000' } };
    for (let i = 0; i < maxRows; i++) {
        const rowData = {};
        for (const header of headerSimplificat) {
            const item = groupedData[header]?.[i];
            if (item) {
                rowData[header.replace(/\s|\./g, '')] = item.cod;
            }
        }
        ...
        addedRow.eachCell({ includeEmpty: false }, (cell, colNumber) => {
            ...
            if (item) {
                cell.font = item.status === 'Stoc Redus' ? redBoldFont : blackBoldFont;
            }
        });
    }
