// --- server.js - COD COMPLET FINAL (cu al 3-lea sheet în format de listă) ---
...
app.post('/process', async (req, res) => { const stocFile = req.file; try { const selectedSuppliers = JSON.parse(req.body.suppliers); const rezultateStoc = await runProcessing(stocFile.path, selectedSuppliers); const rezultateFormatate = Object.keys(rezultateStoc).map(cheie => { const [tipMaterial, descriereRaport] = cheie.split('|'); return { tipMaterial, descriereRaport, cantitate: rezultateStoc[cheie] }; }).filter(item => item.cantitate >= 1); res.json(rezultateFormatate); } catch (error) { console.error('Eroare la /process:', error); res.status(500).send(error.message); } finally { if (stocFile) fs.unlinkSync(stocFile.path); } });
app.post('/download', async (req, res) => { const stocFile = req.file; try { const selectedSuppliers = JSON.parse(req.body.suppliers); const rezultateStoc = await runProcessing(stocFile.path, selectedSuppliers); const excelBuffer = await generateExcelReport(rezultateStoc); const numeFisier = `Stoc_Materie_Prima_${new Date().toLocaleDateString('ro-RO').replace(/\./g, '-')}.xlsx`; res.setHeader('Content-Disposition', `attachment; filename="${numeFisier}"`); res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'); res.send(excelBuffer); } catch (error) { console.error('Eroare la /download:', error); res.status(500).send(error.message); } finally { if (stocFile) fs.unlinkSync(stocFile.path); } });
app.post('/download-pdf', async (req, res) => { const stocFile = req.file; try { const selectedSuppliers = JSON.parse(req.body.suppliers); const rezultateStoc = await runProcessing(stocFile.path, selectedSuppliers); const pdfBuffer = await generatePdfReport(rezultateStoc); const numeFisier = `Stoc_materie_prima-uz_extern-${new Date().toLocaleDateString('ro-RO').replace(/\./g, '-')}.pdf`; res.setHeader('Content-Disposition', `attachment; filename="${numeFisier}"`); res.setHeader('Content-Type', 'application/pdf'); res.send(pdfBuffer); } catch (error) { console.error('Eroare la /download-pdf:', error); res.status(500).send(error.message); } finally { if (stocFile) fs.unlinkSync(stocFile.path); } });
...
async function generateExcelReport(rezultateStoc) {
    ...
    dateTabel.forEach(row => {
        const materialType = row.tip.trim();
        if (!groupedData[materialType]) {
            groupedData[materialType] = [];
        }
        groupedData[materialType].push({ cod: row.cod, status: row.status });
    });
    ...
    for (let i = 0; i < maxRows; i++) {
        ...
        addedRow.eachCell({ includeEmpty: false }, (cell, colNumber) => {
            const headerName = worksheet3.getColumn(colNumber).key;
            const item = groupedData[headerName]?.[i];
            if (item) {
                cell.font = item.status === 'Stoc Redus' ? redBoldFont : blackFont;
            }
        });
    }
...
// Funcția PDF rămâne neschimbată
